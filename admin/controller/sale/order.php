...
/* ადმინ პანელში გმაოვიტანოთ თითეულ ორდერში მასზე მონაცემი */
		if ($order_info) {

			$this->load->language('sale/order');
			$tablename = DB_PREFIX."transactions";
			$sqlquery ="
			SELECT transaction_id, customer_id, currency, status FROM `$tablename` WHERE `order_id` = '$order_id'
			";

			$resultate = $this->db->query($sqlquery);
			$data['t_transaction_id'] = empty($resultate->row['transaction_id'])?'no result':$resultate->row['transaction_id'];
			$data['t_status'] = empty($resultate->row['status'])?'no result':$resultate->row['status'];
			$data['t_currency'] = empty($resultate->row['currency'])?'no result':$resultate->row['currency'];
			$data['t_total'] = sprintf("%0.2f",$order_info['total']);
			$data['t_order_id'] = $order_info['order_id'];
			$data['t_customer_id'] = empty($resultate->row['customer_id'])?'no result':$resultate->row['customer_id'];
			$data['t_reverse_transaction_url'] = $this->url->link('payment/tbc/reverse_transaction', 'token=' . $this->session->data['token'], 'SSL');
			$data['t_return_transaction_amount_url'] = $this->url->link('payment/tbc/return_transaction_amount', 'token=' . $this->session->data['token'], 'SSL');
